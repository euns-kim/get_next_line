...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/05 18:05:18 by eunskim           #+#    #+#             */
/*   Updated: 2022/12/06 22:05:02 by eunskim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * THIS MACRO HAS A CONSTANT VALUE.
 * FILE DESCRIPTORS WHICH COULD BE ACTUALLY OPENED
 * COULD BE MORE THAN THE CONSTANT,
 * YET THIS MACRO CONTROLS THE NUMBER OF BITS
 * IN AN FD_SET. */

char	*get_next_line(int fd)
{
	char		*line;
	static char	*backup[FD_SETSIZE];
	ssize_t		index;

	if (fd == -1 || BUFFER_SIZE <= 0)
		return (NULL);
	backup[fd] = read_next_line(fd, backup[fd]);
	line = get_first_line(&index, backup[fd]);
	backup[fd] = backup_the_rest(&index, backup[fd]);
	return (line);
}
...
/**
 * Trims the first line from the backup,
